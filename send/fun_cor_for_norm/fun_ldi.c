...
static	void	fun_ldi_ind_reg_go(t_main *m, t_process *p, int r_2, int r_3)
{
	short int	ind;
	int			d_1;
	int			d_2;

	ind = m->map[(p->index + 1 + 1) % MS];
	ind = ind << 8;
...
static	void	fun_ldi_ind_reg(t_main *m, t_process *p)
{
	int			r_2;
	int			r_3;
	...
	if (r_2 < 0 || r_2 > 15 || r_3 < 0 || r_3 > 15)
	{
		...
		fun_ldi_ind_reg_go(m, p, r_2, r_3);
}

static	void	fun_ldi_ind_dir_go(t_main *m, t_process *p, int r_3)
{
	short int	ind;
	int			d_1;
	short int	d_2;
...
static	void	fun_ldi_ind_dir(t_main *m, t_process *p)
{
	int			r_3;
	...
	if (r_3 < 0 || r_3 > 15)
	{
		...
		fun_ldi_ind_dir_go(m, p, r_3);
}

void			fun_ldi(t_main *m, t_process *p)
{
