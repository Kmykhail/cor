...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/22 10:15:12 by ozharko           #+#    #+#             */
/*   Updated: 2018/05/22 10:15:14 by ozharko          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fun_and_reg_ind(t_main *m, t_process *p)
{
	...
	p->rg[r_3] = p->rg[r_3] << 8;
	p->rg[r_3] = p->rg[r_3] | (p->rg[r_1] & m->map[(ind + 1) % MS]);
	p->rg[r_3] = p->rg[r_3] << 8;
	p->rg[r_3] = p->rg[r_3] | (p->rg[r_1] & m->map[(ind + 2) % MS]);
	p->rg[r_3] = p->rg[r_3] << 8;
	p->rg[r_3] = p->rg[r_3] | (p->rg[r_1] & m->map[(ind + 3) % MS]);
	p->carry = (p->rg[r_3]) ? 0 : 1;
}
